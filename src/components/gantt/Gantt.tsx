...
export function Gantt({ rows, milestones, expenses = [], people, currency, canEdit, projectId, projectStart, projectEnd, mode }: {
  ...
  const cols = mode === "project" ? "grid-cols-[62px_minmax(200px,1fr)_120px_58px_100px_100px]" : "grid-cols-[18px_minmax(220px,1fr)_120px_58px_110px_110px]";
  const leftW = mode === "project" ? "w-[640px]" : "w-[636px]";
  const stickyTop = { position: "sticky" as const, top: 0 };
  ...
  return (
    <div className="card overflow-hidden">
      ...
      {/* Zone de defilement : en-tetes figes en haut, colonnes figees a gauche */}
      <div className="relative overflow-auto" style={{ maxHeight: "calc(100vh - 210px)" }}>
        <div className="flex" style={{ minWidth: "100%" }}>
          <div className={`sticky left-0 z-20 shrink-0 border-r border-line-hair bg-surface ${leftW}`}>
            <div className={`grid ${cols} z-30 items-end border-b border-line-hair bg-thead px-2 pb-1.5 eyebrow`} style={{ ...stickyTop, height: HEAD_H }}>
            ...
            {msRow === 1 && (
              <div className={`grid ${cols} z-30 items-center border-b border-line-light bg-surface-alt px-2 text-[10.5px]`} style={{ position: "sticky", top: HEAD_H, height: ROW_H }}>
              ...
              return (
                <div key={r.id} className={`grid ${cols} items-center border-b border-line-light px-2 text-[10.5px] ${isLot ? "bg-surface-alt font-bold" : "hover:bg-surface-alt"}`} style={{ height: ROW_H }}>
                  <div className="flex items-center gap-1.5 truncate pr-1">
                    ...
                    {mode === "project" && <span className={`font-mono text-[9.5px] ${isLot ? "text-ink" : "text-ink-faint"}`}>{r.code}</span>}
                  </div>
          ...
          <div className="relative shrink-0" style={{ width, height: Math.max(gridH, HEAD_H + ROW_H) }}>
            <div className="z-10 border-b border-line-hair bg-thead" style={{ ...stickyTop, height: HEAD_H }}>
            ...
            {msRow === 1 && <div className="z-10 border-b border-line-light bg-surface-alt" style={{ position: "sticky", top: HEAD_H, height: ROW_H }}>
              {milestones.map((m) => <Diamond key={m.id} m={m} left={x(m.due) + px / 2} top={ROW_H / 2} t0={t0} onClick={() => canEdit && setSelMilestone(m)} />)}
            </div>}
            {ticks.map((t, i) => <div key={i} className={`absolute bottom-0 border-r ${t.major ? "border-line-hair" : "border-line-light"}`} style={{ left: t.left + t.width - 1, top: HEAD_H + msRow * ROW_H }} />)}
            {visible.map((_, i) => <div key={i} className="absolute left-0 right-0 border-b border-line-light" style={{ top: rowTop(i), height: ROW_H }} />)}
            {todayX >= 0 && todayX <= width && (
              <div className="absolute bottom-0 z-[6] w-px bg-brand" style={{ left: todayX, top: HEAD_H + msRow * ROW_H }}>
                <div className="absolute -left-[18px] top-0 rounded-xs bg-brand px-1 text-[8px] font-bold leading-[13px] text-surface">Auj.</div>
              </div>
            )}
            {links.length > 0 && (
      ...
      {selected && projectId && (
        <TaskDrawer task={selected === "new" ? null : (selected.task ?? null)} isLot={selected !== "new" && selected.kind === "lot"} lots={lots.map((l) => ({ id: l.id, name: l.name }))}
          ...
          people={people} currency={currency} projectId={projectId} canEdit={canEdit} defaults={{ start: projectStart, end: projectEnd }} spent={selected === "new" ? 0 : selected.spent} onClose={() => setSelected(null)} />
      )}
