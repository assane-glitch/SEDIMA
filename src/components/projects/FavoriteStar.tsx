...
/** Etoile de favori : anthracite pleine quand le projet est en favori, contour gris sinon. */
export function FavoriteStar({ projectId, favorite, className = "" }: { projectId: string; favorite: boolean; className?: string }) {
  ...
  return (
    <button type="button" disabled={pending} aria-pressed={fav} title={fav ? "Retirer des favoris" : "Ajouter aux favoris"}
      ...
      <svg viewBox="0 0 24 24" className={`h-4 w-4 ${fav ? "fill-ink text-ink" : "fill-none text-ink-faint hover:text-ink"}`} stroke="currentColor" strokeWidth={1.8} strokeLinejoin="round"><path d="m12 2.8 2.9 6 6.6.9-4.8 4.6 1.2 6.5L12 17.7l-5.9 3.1 1.2-6.5L2.5 9.7l6.6-.9z" /></svg>
    </button>
  );
}
