  ...
  return (
    <div ref={ref} className="relative">
      <button type="button" onClick={() => setOpen((v) => !v)} aria-haspopup="menu" aria-expanded={open} aria-label="Menu du profil" title={name}
        className={`flex h-8 cursor-pointer items-center gap-2 rounded-full pl-1 pr-2 transition-colors hover:bg-surface-sub ${open ? "bg-surface-sub" : ""}`}>
        <span className="flex h-6 w-6 items-center justify-center rounded-full bg-ink text-[10px] font-bold text-surface">{name.slice(0, 1).toUpperCase()}</span>
