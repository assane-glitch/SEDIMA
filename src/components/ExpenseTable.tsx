...
import { excludedStatuses, labelOf, toneOf, type RefItem } from "@/lib/reference-types";
...
export function ExpenseTable({ expenses, currency, taskName, projectName, canEdit, showProject, categories, statuses }: {
  expenses: Expense[]; currency: string; taskName: Map<string, string>; projectName?: Map<string, string>; canEdit: boolean; showProject?: boolean; categories: RefItem[]; statuses: RefItem[];
}) {
  ...
  return (
    <div className="card overflow-x-auto">
      <table className="tbl">
        ...
        <tbody>
          {expenses.map((e) => (
            <tr key={e.id} className={excluded.has(e.status) ? "opacity-60" : ""}>
              ...
              <td className="text-ink-muted">{labelOf(categories, e.category)}</td>
              ...
              <td>
                {canEdit ? (
                  <form action={updateExpense} className="flex items-center gap-1">
                    ...
                    <select name="status" defaultValue={e.status} className="input !w-auto !py-[2px] !text-[9.5px]">{statuses.map((st) => <option key={st.value} value={st.value}>{st.label}</option>)}</select>
                    <button className="btn-secondary !px-1.5 !py-[2px]" title="Mettre a jour le statut">✓</button>
                  </form>
                ) : <Badge tone={toneOf(statuses, e.status)}>{labelOf(statuses, e.status)}</Badge>}
              </td>
          ...
          {expenses.length > 0 && <tr><td colSpan={3} className="font-bold">Total hors statuts exclus</td><td className="num font-bold">{formatMoney(total, currency)}</td><td colSpan={showProject ? 7 : 6} /></tr>}
        </tbody>
      </table>
    </div>
  );
