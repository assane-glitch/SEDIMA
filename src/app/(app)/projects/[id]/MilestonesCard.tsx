    ...
    <section className="card p-4">
      ...
      <ul className="space-y-1.5 text-[10.5px]">
        {milestones.map((m) => {
          ...
          return (
            <li key={m.id} className="group flex items-center gap-2 rounded-lg px-1 py-1 hover:bg-surface-alt">
              {canEdit ? (
                <form action={toggleMilestone}>
                  ...
                  <button title={m.reached_on ? "Marquer non atteint" : "Marquer atteint"} className={`flex h-5 w-5 items-center justify-center rounded-md border ${m.reached_on ? "border-ok bg-ok text-white" : overdue ? "border-alert" : "border-line"}`}>
                    {m.reached_on && <Icon name="check" className="h-3 w-3" strokeWidth={3} />}
                  </button>
                </form>
              ) : (
                <Icon name={m.reached_on ? "check" : "flag"} className={`h-4 w-4 ${m.reached_on ? "text-ok" : overdue ? "text-ink" : "text-ink-faint"}`} />
              )}
              <span className={`flex-1 truncate ${m.reached_on ? "text-ink-faint line-through" : ""}`} title={m.notes || undefined}>{m.name}</span>
              <span className={`text-[10px] tabular-nums ${overdue ? "font-semibold text-ink" : "text-ink-muted"}`}>{m.reached_on ? `atteint le ${formatDate(m.reached_on)}` : formatDate(m.due_date)}</span>
              {canEdit && (
                <form action={deleteMilestone} className="opacity-0 group-hover:opacity-100">
                  ...
                  <button className="text-ink-ghost hover:text-ink" title="Supprimer"><Icon name="x" className="h-3.5 w-3.5" /></button>
                </form>
              )}
            </li>
          );
