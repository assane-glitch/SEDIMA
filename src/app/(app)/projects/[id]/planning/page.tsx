...
export default async function ProjectPlanningPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: ms } = await supabase.from("milestones").select("*").eq("project_id", id).order("due_date");
  ...
  return (
    <>
      ...
      <Gantt mode="project" rows={rows} milestones={milestones} people={people} currency={project.currency} canEdit={editor} projectId={id} projectStart={project.start_date} projectEnd={project.end_date} />
    </>
  );
}
