  ...
  return (
    <>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((i) => (
          <Link key={i.label} href={i.href} className={`card flex items-center justify-between px-4 py-4 ${i.ready ? "hover:bg-slate-50" : "opacity-60"}`}>
            <div><div className="font-medium">{i.label}</div><div className="text-xs text-slate-500">{i.hint}{i.ready ? "" : " · etape 9"}</div></div>
