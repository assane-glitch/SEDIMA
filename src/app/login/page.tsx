...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string; next?: string; info?: string }> }) {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#1f2937] text-white">
      ...
      <div className="absolute inset-y-0 left-0 w-full bg-[#1f2937]/85 backdrop-blur-[2px] lg:w-[65%] lg:[clip-path:polygon(0_0,100%_0,78%_100%,0_100%)]" />
      <svg className="pointer-events-none absolute inset-y-0 left-0 hidden h-full w-[65%] lg:block" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden><line x1="100" y1="0" x2="78" y2="100" stroke="#fcd814" style={{ strokeWidth: 3 }} vectorEffect="non-scaling-stroke" /></svg>

      {/* Contenu centre dans la zone filtree */}
      <div className="relative flex min-h-screen flex-col lg:w-[65%] lg:pr-[11%]">
          ...
          <div className="w-full max-w-md">
            ...
            <form action={signIn} className="mt-9 space-y-7">
              ...
              <div className="relative">
                <label htmlFor="email" className="block text-center text-[10px] font-semibold uppercase tracking-[0.12em] text-white/60">Email</label>
                <input id="email" name="email" type="email" required autoComplete="email" placeholder="prenom.nom@sedima.sn"
                  className="peer w-full border-0 border-b border-white/30 bg-transparent px-8 py-2 text-center text-[15px] text-white outline-none placeholder:text-center placeholder:text-white/25 focus:border-white" />
              ...
              <div className="relative">
                <label htmlFor="password" className="block text-center text-[10px] font-semibold uppercase tracking-[0.12em] text-white/60">Mot de passe</label>
