  ...
  return (
    <main className="mx-auto max-w-xl px-4 py-8">
      <div className="mb-6 flex items-center justify-between">
        ...
        <Link href="/dashboard" className="text-[10.5px] text-ink-muted hover:text-ink">‹ Retour a l&apos;application</Link>
      </div>
      <h1 className="text-[16px] font-semibold tracking-tight">Mon compte</h1>
      <p className="mt-1 text-[10.5px] text-ink-muted">{profile.email} · {ROLE_LABELS[profile.role]}</p>
      {error && <div className="mt-4"><Alert>{error}</Alert></div>}
      {ok && <div className="mt-4"><Alert tone="green">{ok}</Alert></div>}

      <form action={updateName} className="card mt-6 space-y-4 p-6">
        <h2 className="text-[10.5px] font-semibold">Identite</h2>
        <div>
          ...
          <p className="mt-1 text-[10px] text-ink-muted">Ce nom apparait sur les projets, les taches et dans l&apos;historique. Utilisez la meme orthographe que dans le referentiel pour que vos projets vous soient rattaches.</p>
        </div>
      ...
      <form action={updatePassword} className="card mt-4 space-y-4 p-6">
        <h2 className="text-[10.5px] font-semibold">Mot de passe</h2>
